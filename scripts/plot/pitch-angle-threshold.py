...
for x in list1:
    ...
    # 3 deg
    def f1(c):
        if 0 <= c['pitch'] <= 3 or ((c['pitch'] > 3 or c['pitch'] < 0) and (
                c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
    ...
    # 5 deg
    def f2(c):
        if 0 <= c['pitch'] <= 5 or ((c['pitch'] > 5 or c['pitch'] < 0) and (
                c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
    ...
    # 7 deg
    def f3(c):
        if 0 <= c['pitch'] <= 7 or ((c['pitch'] > 7 or c['pitch'] < 0) and (
                c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
    ...
    # 10 deg
    def f4(c):
        if 0 <= c['pitch'] <= 10 or ((c['pitch'] > 10 or c['pitch'] < 0) and (
                c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
...
for x in list1:
    ...
    # 2.5 deg
    def f5(c): 
        if 0 <= c['pitch'] <= 2.5 or ((c['pitch'] > 2.5 or c['pitch'] < 0) and
                (c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
    ...
    # 3.5 deg
    def f6(c):
        if 0 <= c['pitch'] <= 3.5 or ((c['pitch'] > 3.5 or c['pitch'] < 0) and
                (c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
    ...
    # 4 deg
    def f7(c):
        if 0 <= c['pitch'] <= 4 or ((c['pitch'] > 4 or c['pitch'] < 0) and
                (c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
    ...
    # 4.5 deg
    def f8(c):
        if 0 <= c['pitch'] <= 4.5 or ((c['pitch'] > 4.5 or c['pitch'] < 0) and
                (c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
    ...
    # 5.5 deg
    def f9(c):
        if 0 <= c['pitch'] <= 5.5 or ((c['pitch'] > 5.5 or c['pitch'] < 0) and
                (c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
    ...
    # 6 deg
    def f10(c):
        if 0 <= c['pitch'] <= 6 or ((c['pitch'] > 6 or c['pitch'] < 0) and
                (c['ap_av'] <= (.1 * dfx['ap_av'].max()) or
                c['ap_av'] >= (.9 * dfx['ap_av'].max()))):
            return 'normal'
        else:
...
ax = df2.plot(subplots=True, style='.', layout=(5, 5), figsize=(50, 30),
...
# %%
ax = p.plot(subplots=True, figsize=(50, 30), layout=(5, 5), style='.',
